      ...
      <div className="border-b border-slate-200 pb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            ...
            className="p-2 rounded-xl bg-white border border-slate-200 text-slate-700 hover:text-emerald-600"
            title="Refresh Data"
          >
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 perspective-1000">
            <div className="p-5 rounded-3xl glass-3d-card-light card-3d-tilt space-y-1">
              <span className="text-[11px] text-slate-500 font-mono font-bold uppercase">Spot Price</span>
          ...
          <div className="p-6 rounded-3xl glass-3d-card-light shadow-xl overflow-x-auto">
            <h3 className="text-base font-bold text-slate-900 mb-4">Strike Price Matrix ({indexSymbol})</h3>
            ...
            <table className="w-full text-center text-xs">
              ...
              <tbody className="divide-y divide-slate-200 font-mono">
                {chainData.chain.map((row) => {
                  ...
                  return (
                    <tr key={row.strike} className={`${isATM ? 'bg-emerald-50/70 font-bold' : 'hover:bg-slate-50'}`}>
                      <td className="p-2 text-slate-600">{row.call_oi.toLocaleString()}</td>
                      <td className="p-2 text-slate-500">{row.call_iv}%</td>
                      <td className="p-2 font-bold text-emerald-700">₹{row.call_price}</td>
                      <td className={`p-2 font-black ${isATM ? 'bg-emerald-600 text-white rounded-lg' : 'bg-slate-100 text-slate-900'}`}>
                        {row.strike}
                      </td>
                      <td className="p-2 font-bold text-rose-700">₹{row.put_price}</td>
                      <td className="p-2 text-slate-500">{row.put_iv}%</td>
                      <td className="p-2 text-slate-600">{row.put_oi.toLocaleString()}</td>
                    </tr>
                  );
                })}
              </tbody>
